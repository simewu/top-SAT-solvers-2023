...
static inline bool non_watching_propagate_literal(kissat *solver, unsigned lit, unsigned ignore)
{
...
bool kissat_dense_propagate(kissat *solver, unsigned limit, unsigned ignore_idx)
{
    ...
    bool res = true;
    while (res && generations < limit && solver->propagated < SIZE_STACK(solver->trail)) {
    ...
    LOG("%s %u literals in %u generations %s conflict",
        (solver->propagated == SIZE_STACK(solver->trail)) ? "completely propagated" : "incomplete propagation of",
