...
void kissat_enable_reluctant(reluctant *reluctant, uint64_t period, uint64_t limit)
{
...
void kissat_init_reluctant(kissat *solver)
{
    if (GET_OPTION(reluctant)) {
        LOG("enable reluctant doubling with period %d limit %d", GET_OPTION(reluctantint), GET_OPTION(reluctantlim));
        kissat_enable_reluctant(&solver->reluctant, GET_OPTION(reluctantint), GET_OPTION(reluctantlim));
    } else {
