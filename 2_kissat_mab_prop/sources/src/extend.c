...
static void undo_eliminated_assignment(kissat *solver)
{
    ...
    LOG("unassigning %zu eliminated variables %.0f%%", size_etrail, kissat_percent(size_etrail, size_eliminated));
...
void kissat_extend(kissat *solver)
{
    ...
    LOG("extending solution with reconstruction stack of size %zu", SIZE_STACK(solver->extend));
    ...
    LOG("assigned %zu external variables %.0f%% out of %zu eliminated", assigned, kissat_percent(assigned, total),
        total);
    LOG("flipped %zu external variables %.0f%% out of %zu assigned", flipped, kissat_percent(flipped, assigned),
        assigned);
