...
static void activate_literal(kissat *solver, unsigned lit)
{
    ...
    if (solver->mab) {
        ...
        kissat_push_heap(solver, solver->heuristic == 0 ? &solver->scores : &solver->scores_chb, idx);
...
static void deactivate_variable(kissat *solver, flags *f, unsigned idx)
{
