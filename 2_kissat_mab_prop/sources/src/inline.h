...
static inline void kissat_push_large_watch(kissat *solver, watches *watches, reference ref)
{
    const watch watch = kissat_large_watch(ref);
    PUSH_WATCHES(*watches, watch);
}

static inline void kissat_push_binary_watch(kissat *solver, watches *watches, bool redundant, bool hyper,
    unsigned other)
{
...
static inline void kissat_push_blocking_watch(kissat *solver, watches *watches, unsigned blocking, reference ref)
{
...
static inline void kissat_watch_other(kissat *solver, bool redundant, bool hyper, unsigned lit, unsigned other)
{
    LOGBINARY(lit, other, "watching %s blocking %s in %s", LOGLIT(lit), LOGLIT(other),
        (redundant ? "redundant" : "irredundant"));
    watches *watches = &WATCHES(lit);
    kissat_push_binary_watch(solver, watches, redundant, hyper, other);
}

static inline void kissat_watch_binary(kissat *solver, bool redundant, bool hyper, unsigned a, unsigned b)
{
    kissat_watch_other(solver, redundant, hyper, a, b);
    kissat_watch_other(solver, redundant, hyper, b, a);
}

static inline void kissat_watch_blocking(kissat *solver, unsigned lit, unsigned blocking, reference ref)
{
    assert(solver->watching);
...
static inline void kissat_unwatch_blocking(kissat *solver, unsigned lit, reference ref)
{
    assert(solver->watching);
...
static inline void kissat_disconnect_binary(kissat *solver, unsigned lit, unsigned other)
{
    assert(!solver->watching);
    watches *watches = &WATCHES(lit);
...
static inline void kissat_disconnect_reference(kissat *solver, unsigned lit, reference ref)
{
    assert(!solver->watching);
...
static inline void kissat_connect_literal(kissat *solver, unsigned lit, reference ref)
{
    assert(!solver->watching);
...
static inline clause *kissat_unchecked_dereference_clause(kissat *solver, reference ref)
{
...
static inline reference kissat_reference_clause(kissat *solver, const clause *c)
{
    assert(kissat_clause_in_arena(solver, c));
    return (word *)c - BEGIN_STACK(solver->arena);
}

static inline void kissat_inlined_connect_clause(kissat *solver, watches *all_watches, clause *c, reference ref)
{
    assert(!solver->watching);
...
static inline void kissat_update_queue(kissat *solver, const links *links, unsigned idx)
{
...
static inline unsigned kissat_map_literal(kissat *solver, unsigned ilit, bool map)
{
    if (!map)
        return ilit;
...
static inline void kissat_update_variable_score(kissat *solver, heap *schedule, unsigned idx)
{
    assert(schedule->size);
    const bool esa = false;
    if(esa) {
        kissat_update_heap(solver, schedule, idx, -kissat_get_heap_score(solver->heuristic == 0 ? & solver->scores : & solver->scores_chb, idx));
    }else {
        const unsigned lit = LIT(idx);
        const unsigned not_lit = NOT(lit);
        size_t pos = WATCHES(lit).size;
        size_t neg = WATCHES(not_lit).size;
        double new_score = ((double)pos) * neg + pos + neg; // + kissat_get_heap_score (&(solver->scores), idx);
        LOG("new elimination score %g for variable %u (pos %zu and neg %zu)", new_score, idx, pos, neg);
...
{
    return (solver->last_irredundant == INVALID_REF) ? 0 : kissat_dereference_clause(solver, solver->last_irredundant);
}

static inline clause *kissat_binary_conflict(kissat *solver, bool redundant, unsigned a, unsigned b)
{
