...
static unsigned determine_new_level(kissat *solver, unsigned jump)
{
    ...
    const unsigned limit = GET_OPTION(chrono) ? (unsigned)GET_OPTION(chronolevels) : UINT_MAX;
    ...
    if (!delta) {
        ...
        res = back;
        LOG("backjumping over %u levels (%u - %u) considered inefficient", delta, back, jump);
        LOG("backtracking chronologically to backtrack level %u", res);
        INC(chronological);
    } else {
        res = jump;
        LOG("backjumping over %u levels (%u - %u) considered efficient", delta, back, jump);
...
static void learn_unit(kissat *solver)
{
...
static void learn_binary(kissat *solver)
{
    const unsigned *lits = BEGIN_STACK(solver->clause.lits);
    const unsigned not_uip = lits[0];
...
static void learn_reference(kissat *solver)
{
...
void kissat_learn_clause(kissat *solver)
{
    ...
    LOG("learned[%" PRIu64 "] clause glue %u size %u", GET(learned), glue, size);
    if (!solver->probing) {
...
static void pr_learn_unit(kissat *solver)
{
    const unsigned *lits = BEGIN_STACK(solver->clause.lits);
    const unsigned not_uip = lits[0];
...
static void pr_learn_binary(kissat *solver)
{
    kissat_backtrack(solver, 0);
    kissat_new_irredundant_clause(solver);
}

static void pr_learn_reference(kissat *solver)
{
    kissat_new_irredundant_clause(solver);
    kissat_backtrack(solver, 0);
}

int kissat_learn_pr(kissat *solver) {
    const unsigned size = SIZE_STACK(solver->clause.lits);
    unsigneds c;
    INIT_STACK(c);
    for(unsigned i = 0; i < SIZE_STACK(solver->clause.lits); i++) {
        PUSH_STACK(c, PEEK_STACK(solver->clause.lits, i));
    }
    PUSH_STACK(solver->clauses, c);
    if (size == 1) {
        pr_learn_unit(solver);
    } else if (size == 2) {
        pr_learn_binary(solver);
    } else {
        pr_learn_reference(solver);
    }
    CLEAR_STACK(solver->clause.lits);
    if(kissat_search_propagate(solver)) {
        return 20;
    }else {
        return 10;
    }
