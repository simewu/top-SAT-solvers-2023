...
#define FORMAT_SIGNS(SIZE, SIGNS) kissat_format_signs(&solver->format, SIZE, SIGNS)

#define FORMAT_TIME(SECONDS) kissat_format_time(&solver->format, SECONDS)

#define FORMAT_VALUE(BOOLEAN, VALUE) kissat_format_value(&solver->format, BOOLEAN, VALUE)
