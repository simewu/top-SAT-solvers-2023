...
void kissat_release_proof(struct kissat *, int);
...
void kissat_shrink_clause_in_proof(struct kissat *, struct clause *, unsigned remove, unsigned keep);
...
void print_added_proof_line(proof *);
void print_delete_proof_line(proof *);
void kissat_generate_preorder(kissat *, int, int);
void kissat_derive_lex_order(kissat *, ints *, int);
...
#define ADD_STACK_TO_PROOF(S) ADD_LITS_TO_PROOF(SIZE_STACK(S), BEGIN_STACK(S))
...
#define DELETE_STACK_FROM_PROOF(S)                                                    \
    do {                                                                              \
        if (solver->proof)                                                            \
            kissat_delete_internal_from_proof(solver, SIZE_STACK(S), BEGIN_STACK(S)); \
    } while (0)
