...
#define OPTIONS                                                                  \
    ...
    OPTION(geneseedcount, 100, 0, 10000, "number of seeds in genetic algoritm")  \
    ...
    OPTION(mab, 1, 0, 1, "enable MAB between VSIDS and CHB")                     \
    ...
    OPTION(modecdecimal, 0, 0, 9, "mode mab const decimal")                      \
    OPTION(modecint, 4, 0, 10, "mode mab const floor")                           \
    OPTION(modeinit, 1e3, 10, 1e8, "initial mode change interval")               \
    OPTION(modeint, 1e3, 10, 1e8, "base mode change interval")                   \
    OPTION(modemab, 1, 0, 1, "enable MAB between luby and EMA")                  \
    ...
    OPTION(pr, 1, 0, 1, "enable pr preprocessing")                               \
    ...
    OPTION(propmab, 1, 0, 1, "use number of propagations to calculate score of MAB, 1: use, 2: use number of decisions instead") \
    ...
    OPTION(reluctantint, 1 << 10, 2, 1 << 20, "reluctant interval")              \
    OPTION(reluctantlim, 1 << 20, 0, 1 << 30, "reluctant limit (0=unlimited)")   \
    OPTION(rephase, 1, 0, 1, "reinitialization of decision phases")              \
    OPTION(rephaseinit, 1000, 10, 1e5, "initial rephase interval")                \
    OPTION(rephaseint, 1000, 10, 1e5, "base rephase interval")                    \
    ...
    OPTION(symmetry, 1, 0, 1, "detect symmetry when possible")                   \
    ...
    OPTION(timeout, 0, 0, 300000, "timelimit. 0 for no limit")                \
    ...
    OPTION(walkbits, 16, 1, 1e5, "number of enumerated variables per walk step") \
    ...
    OPTION(walkstrategy, 3, 0, 3, "walk (local search) strategy. 0: default, 1: no walk, 2: 5 times walk, 3: select by decision tree") \
...
#define all_options(O)                   \
    const opt *O = kissat_options_begin; \
