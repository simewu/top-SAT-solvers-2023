...
static void reassign_queue_stamps(kissat *solver, links *links, queue *queue)
{
...
static inline void enqueue(kissat *solver, unsigned i, links *links, queue *queue)
{
...
static inline void dequeue(unsigned i, links *links, queue *queue)
{
...
void kissat_check_queue(kissat *solver)
{
    ...
    for (unsigned idx = queue->first, prev = DISCONNECT; !DISCONNECTED(idx); idx = links[idx].next) {
