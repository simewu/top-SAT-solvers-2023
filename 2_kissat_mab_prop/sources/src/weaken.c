...
#define LOGPUSHED(SIZE)                                                                                  \
    do {                                                                                                 \
        LOGEXT((SIZE), END_STACK(solver->extend) - (SIZE), "pushed size %zu witness labelled clause at", \
            (size_t)(SIZE));                                                                             \
    } while (0)
...
void kissat_weaken_unit(kissat *solver, unsigned lit)
{
    ...
    push_witness_literal(solver, lit);
    LOGEXT(1, END_STACK(solver->extend) - 1, "pushed witness labelled unit clause at");
}
