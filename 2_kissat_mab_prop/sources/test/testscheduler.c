...
tissat_job *tissat_schedule_command(int expected, const char *command, tissat_job *dependency)
{
    tissat_job *res = new_job(expected);
...
static void check_command(tissat_job *job, int status)
{
    assert(job);
    assert(job->command);
    if (WIFEXITED(status)) {
        ...
        if (res == job->expected)
            tissat_verbose("Command '%s' returned '%d' as expected.", job->command, res);
        else
            tissat_error("Command '%s' returns '%d' and not '%d'", job->command, res, job->expected);
...
static void check_function(tissat_job *job, int status)
{
    ...
    assert(job->function);
    if (status)
        tissat_error("Function job '%s' failed with exit status '%d", job->name, status);
}
...
static void check_application(tissat_job *job, int status)
{
    ...
    if (status)
        tissat_error("Application job 'kissat %s' failed with exit status '%d", job->application, status);
}

static void execute_command(tissat_job *job)
{
...
static void execute_job(tissat_job *job)
{
    //printf("%s %s %s\n", job->function, job->application, job->command);
    running_job = job;
    if (job->function)
...
double kissat_process_time();
static void run_jobs_sequentially(void)
{
    tissat_message("Running %u jobs sequentially all in the same process.", tissat_scheduled);
    for (all_jobs(job)) {
        printf("(seq)run new job %s %s %s\n", job->command, job->application, job->name);
...
{
    while (assert(search_executed < tissat_scheduled), jobs[search_executed].executed)
        search_executed++;
...
unsigned long mem_avail()
{
  ...
  if (sysinfo(&info) < 0)
    return 0;
    //printf("%lld %lld %lld %lld\n", info.freeram, info.bufferram, info.sharedram, info.freeswap);
...
static void run_jobs_in_parallel(unsigned parallel)
{
    if (parallel == UINT_MAX)
        ...
        tissat_message("Running %u jobs in parallel using up to %d processes.", tissat_scheduled, parallel);
    unsigned running = 0;
    //parallel -= 2;
    //parallel = 2;
    //parallel = 30;
    ...
    while (finished < tissat_scheduled) {
        tissat_job *job;
        //printf("processed %lld %d %d\n", mem_avail(), executed, running);
        if (running < parallel && executed < tissat_scheduled && mem_avail() >= 1 && (job = find_executable_job())) {
            //printf("%lld %d %d\n", get_free_memory(), executed, running);
            //printf("forking new job %s %s %s %g\n", job->command, job->application, job->name, alltime());
            job->pid = fork();
            if (job->pid < 0)
                ...
                //printf("ohh\n");
                printf("%s,%g,\n", job->application, kissat_process_time());
                //ystem("sh -c 'echo 2 > /proc/sys/vm/dropcaches'");
                exit(0);
            } else {
                job->executed = true;
                executed++;
            ...
            if (pid < 0)
                tissat_fatal("waiting on %u unfinished processes failed", executed - finished);
            //printf("waited\n");
            ...
            if (WIFSIGNALED(status))
                ...
                tissat_fatal("unexpected status '%d' of child process '%d'", status, pid);
            assert(other);
            other->finished = true;
            finished++;
            //printf("one job has finished\n");
            assert(running);
            running--;
            parallel_progress(running);
        }
