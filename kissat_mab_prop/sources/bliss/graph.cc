...
void AbstractGraph::refine_to_equitable(Partition::Cell * const unit_cell1, Partition::Cell * const unit_cell2)
{
        ...
        if (cell->is_unit()) {
            if (in_search) {
                ...
                if (first_path_automorphism) {
                    ...
                    first_path_automorphism[first_path_labeling_inv[index]] = p.elements[index];
                }
                if (best_path_automorphism) {
                    /* Build the (potential) automorphism on-the-fly */
                    best_path_automorphism[best_path_labeling_inv[index]] = p.elements[index];
                }
            }
...
bool AbstractGraph::is_automorphism(const std::vector<unsigned int> &perm) const
{
    _INTERNAL_ERROR();
    return false;
}
...
void AbstractGraph::cert_add(const unsigned int v1, const unsigned int v2, const unsigned int v3)
{
...
void AbstractGraph::cert_add_redundant(const unsigned int v1, const unsigned int v2, const unsigned int v3)
{
...
void AbstractGraph::long_prune_init()
{
    ...
    const unsigned int nof_fitting_in_max_mem = (long_prune_options_max_mem * 1024 * 1024) / (((N * 2) / 8) + 1);
...
std::vector<bool> &AbstractGraph::long_prune_allocget_fixed(const unsigned int index)
{
...
std::vector<bool> &AbstractGraph::long_prune_allocget_mcrs(const unsigned int index)
{
    const unsigned int i = index % long_prune_max_stored_autss;
...
void AbstractGraph::update_orbit_information(Orbit &o, const unsigned int *perm)
{
    const unsigned int N = get_nof_vertices();
    for (unsigned int i = 0; i < N; i++)
        if (perm[i] != i)
...
void AbstractGraph::search(const bool canonical, Stats &stats)
{
     ...
     * The actual backtracking search
     */
    while (!search_stack.empty() && timer1.get_duration() < 30) { // timer!
        ...
        if (opt_use_comprec) {
            ...
            if (cep.first_checked == true and current_node.fp_extendable == TreeNode::MAYBE and
        ...
        if (current_node.fp_on) {
            ...
            if (current_node.fp_extendable == TreeNode::NO and (!canonical or current_node.cmp_to_best_path < 0)) {
                if (opt_use_failure_recording) {
                    TreeNode &parent_node = search_stack[current_level - 1];
                    if (parent_node.fp_on)
                        failure_recording_hashes[current_level - 1].insert(current_node.failure_recording_ival);
        ...
        Partition::Cell * const cell = p.get_cell(p.elements[current_node.split_cell_first]);
        ...
        if (opt_use_long_prune and current_level >= 1 and !current_node.fp_on) {
            unsigned int begin =
                (current_node.long_prune_begin > long_prune_begin) ? current_node.long_prune_begin : long_prune_begin;
...
#endif
                if (fixed[search_stack[search_stack.size() - 1 - 1].split_element] == false) {
         ...
         */
        {
            ...
            if (current_node.fp_on) {
                ...
                for (unsigned int i = cell->length; i > 0; i--, ep++) {
                    if ((int)(*ep) > current_node.split_element and *ep < next_split_element and
                ...
                for (unsigned int i = cell->length; i > 0; i--, ep++) {
                    if ((int)(*ep) > current_node.split_element and *ep < next_split_element and
                        ...
                        current_node.long_prune_redundant.find(*ep) == current_node.long_prune_redundant.end())) {
                        next_split_element = *ep;
                        // next_split_element_pos = ep;
                    }
                }
            ...
            if (next_split_element == UINT_MAX) {
                ...
                if (current_node.fp_on) {
                    ...
                    const unsigned int index =
                        first_path_orbits.orbit_size(first_path_info[search_stack.size() - 1].splitting_element);
                    ...
                    if (verbstr and verbose_level >= 2) {
                        fprintf(verbstr, "Level %u: orbits=%u, index=%u/%u, all_same_level=%u\n", current_level,
                            first_path_orbits.nof_orbits(), index, cell->length, all_same_level);
                        fflush(verbstr);
                    }
                }
                continue;
        ...
        if (!first_path_info.empty()) {
            if (refine_equal_to_first)
                refine_first_path_subcertificate_end = first_path_info[search_stack.size() - 1].certificate_index +
                    first_path_info[search_stack.size() - 1].subcertificate_length;
            if (canonical) {
                if (refine_cmp_to_best == 0)
                    refine_best_path_subcertificate_end = best_path_info[search_stack.size() - 1].certificate_index +
        ...
        Partition::Cell * const new_cell = p.individualize(cell, current_node.split_element);
        ...
        if (!first_path_info.empty()) {
            ...
            const unsigned int subcertificate_length = certificate_current_path.size() - certificate_index;
            if (refine_equal_to_first) {
            ...
            if (opt_use_failure_recording and was_fp_cert_equal and !refine_equal_to_first) {
                ...
                if (current_node.fp_on)
                    ...
                    for (unsigned int i = current_level; i > 0; i--) {
                        ...
                        if (i == current_level and s.find(failure_recording_fp_deviation) != s.end())
                            break;
                        if (s.find(0) != s.end())
                            break;
             ...
             * current best path */
            if (refine_equal_to_first == false and (!canonical or (refine_cmp_to_best < 0))) {
                /* Yes, backtrack */
                stats.nof_bad_nodes++;
                if (current_node.fp_cert_equal == true and current_level + 1 > all_same_level) {
        ...
        if (current_node.fp_extendable == TreeNode::NO or
            (current_node.fp_extendable == TreeNode::MAYBE and child_node.fp_cert_equal == false))
        ...
        current_node.subcertificate_length = certificate_index - current_node.certificate_index;
        ...
        if (p.is_discrete() and first_path_info.empty()) {
             ...
             */
            update_labeling_and_its_inverse(first_path_labeling, first_path_labeling_inv);
        ...
        if (!p.is_discrete()) {
             ...
             */
            if (opt_use_comprec) {
                assert(p.nof_discrete_cells() <= cr_cep_stack[cr_cep_index].discrete_cell_limit);
                assert(cr_level == child_node.cr_level);


                if (p.nof_discrete_cells() == cr_cep_stack[cr_cep_index].discrete_cell_limit) {
                    ...
                    if (canonical and !first_path_info.empty() and child_node.cmp_to_best_path >= 0) {
                        if (cep.best_checked == false) {
                            ...
                            if (child_node.cmp_to_best_path == 0) {
                                ...
                                if (old_info.in_best_path) {
                                    ...
                                    for (unsigned int i = postfix_start; i < best_path_temp.size(); i++) {
                                    ...
                                    assert(certificate_best_path.size() == best_path_info.back().certificate_index +
                ...
                if (nucr_find_first_component(cr_level, cr_component, cr_component_elements, next_split_cell) ==
                    true and
                    p.nof_discrete_cells() + cr_component_elements < cr_cep_stack[cr_cep_index].discrete_cell_limit) {
                    const unsigned int next_cr_level = p.cr_split_level(cr_level, cr_component);
                    CR_CEP cep;
    ...
    handle_best_path_automorphism : {
        ...
        if (p.is_discrete()) {
            ...
            for (unsigned int i = 0; i < N; i++)
                assert(best_path_automorphism[i] == p.elements[best_path_labeling[i]]);
#endif
        } else {
            ...
            for (Partition::Cell *c = p.first_nonsingleton_cell; c; c = c->next_nonsingleton) {
        ...
        if (nof_old_orbits != first_path_orbits.nof_orbits()) {
            ...
            /* Report automorphism */
            if (report_hook)
                (*report_hook)(report_user_param, get_nof_vertices(), best_path_automorphism);
            /* Update statistics */
    ...
    handle_first_path_automorphism:
        ...
        if (p.is_discrete()) {
            ...
            for (unsigned int i = 0; i < N; i++)
                assert(first_path_automorphism[i] == p.elements[first_path_labeling[i]]);
#endif
        } else {
            /* An automorphism that was found before the partition was discrete.
             * Set the image of all elements in non-disrete cells accordingly */
        ...
        if (report_hook)
            (*report_hook)(report_user_param, get_nof_vertices(), first_path_automorphism);
...
void Digraph::Vertex::remove_duplicate_edges(std::vector<bool> &tmp)
{
    ...
    for (unsigned int i = 0; i < tmp.size(); i++)
        assert(tmp[i] == false);
#endif
    for (std::vector<unsigned int>::iterator iter = edges_out.begin(); iter != edges_out.end();) {
    ...
    /* Clear tmp */
    for (std::vector<unsigned int>::iterator iter = edges_out.begin(); iter != edges_out.end(); iter++) {
        tmp[*iter] = false;
    }

    for (std::vector<unsigned int>::iterator iter = edges_in.begin(); iter != edges_in.end();) {
        const unsigned int dest_vertex = *iter;
        if (tmp[dest_vertex] == true) {
            /* A duplicate edge found! */
    ...
    /* Clear tmp */
    for (std::vector<unsigned int>::iterator iter = edges_in.begin(); iter != edges_in.end(); iter++) {
        tmp[*iter] = false;
    }
#if defined(BLISS_CONSISTENCY_CHECKS)
    /* Post-conditions  */
...
void Digraph::change_color(const unsigned int vertex, const unsigned int new_color)
{
...
Digraph *Digraph::permute(const std::vector<unsigned int> &perm) const
{
    ...
    for (unsigned int i = 0; i < get_nof_vertices(); i++) {
        ...
        for (std::vector<unsigned int>::const_iterator ei = v.edges_out.begin(); ei != v.edges_out.end(); ei++) {
...
void Digraph::write_dot(FILE * const fp)
{
    ...
    unsigned int vnum = 0;
    for (std::vector<Vertex>::const_iterator vi = vertices.begin(); vi != vertices.end(); vi++, vnum++) {
        const Vertex &v = *vi;
...
void Digraph::remove_duplicate_edges()
{
    ...
    for (std::vector<Vertex>::iterator vi = vertices.begin(); vi != vertices.end(); vi++) {
...
unsigned int Digraph::vertex_color_invariant(const Digraph * const g, const unsigned int vnum)
{
    return g->vertices[vnum].color;
}

unsigned int Digraph::indegree_invariant(const Digraph * const g, const unsigned int vnum)
{
    return g->vertices[vnum].nof_edges_in();
}

unsigned int Digraph::outdegree_invariant(const Digraph * const g, const unsigned int vnum)
{
    return g->vertices[vnum].nof_edges_out();
}

unsigned int Digraph::selfloop_invariant(const Digraph * const g, const unsigned int vnum)
{
...
bool Digraph::refine_according_to_invariant(unsigned int (*inv)(const Digraph * const g, const unsigned int v))
{
    bool refined = false;
...
bool Digraph::split_neighbourhood_of_unit_cell(Partition::Cell * const unit_cell)
{
    ...
    for (unsigned int j = v.nof_edges_out(); j > 0; j--) {
        ...
        unsigned int * const swap_position =
            p.elements + neighbour_cell->first + neighbour_cell->length - neighbour_cell->max_ival_count;
...
bool Digraph::is_equitable() const
{
    ...
    for (Partition::Cell *cell = p.first_cell; cell; cell = cell->next) {
        ...
        for (std::vector<unsigned int>::const_iterator ei = first_vertex.edges_out.begin();
        ...
        for (unsigned int i = cell->length; i > 1; i--) {
            ...
            for (std::vector<unsigned int>::const_iterator ei = vertex.edges_out.begin(); ei != vertex.edges_out.end();
                ei++) {
    ...
    for (Partition::Cell *cell = p.first_cell; cell; cell = cell->next) {
        ...
        for (std::vector<unsigned int>::const_iterator ei = first_vertex.edges_in.begin();
        ...
        for (unsigned int i = cell->length; i > 1; i--) {
            const Vertex &vertex = vertices[*ep++];
            for (std::vector<unsigned int>::const_iterator ei = vertex.edges_in.begin(); ei != vertex.edges_in.end();
                ei++) {
                other_count[p.get_cell(*ei)->first]++;
            }
            for (Partition::Cell *cell2 = p.first_cell; cell2; cell2 = cell2->next) {
...
Partition::Cell *Digraph::sh_first_largest_max_neighbours()
{
    ...
    for (Partition::Cell *cell = p.first_nonsingleton_cell; cell; cell = cell->next_nonsingleton) {
        ...
        if ((value > best_value) || (value == best_value && cell->length > best_size)) {
            best_value = value;
            best_size = cell->length;
            best_cell = cell;
        }
...
bool Digraph::is_automorphism(unsigned int * const perm)
{
    ...
    for (unsigned int i = 0; i < get_nof_vertices(); i++) {
        ...
        edges1.clear();
        for (std::vector<unsigned int>::iterator ei = v1.edges_in.begin(); ei != v1.edges_in.end(); ei++)
            edges1.insert(perm[*ei]);
        edges2.clear();
        for (std::vector<unsigned int>::iterator ei = v2.edges_in.begin(); ei != v2.edges_in.end(); ei++)
            edges2.insert(*ei);
        if (!(edges1 == edges2))
            return false;

        edges1.clear();
        for (std::vector<unsigned int>::iterator ei = v1.edges_out.begin(); ei != v1.edges_out.end(); ei++)
            edges1.insert(perm[*ei]);
        edges2.clear();
        for (std::vector<unsigned int>::iterator ei = v2.edges_out.begin(); ei != v2.edges_out.end(); ei++)
            edges2.insert(*ei);
        if (!(edges1 == edges2))
            return false;
    }
...
bool Digraph::is_automorphism(const std::vector<unsigned int> &perm) const
{
    ...
    for (unsigned int i = 0; i < get_nof_vertices(); i++) {
        ...
        edges1.clear();
        for (std::vector<unsigned int>::const_iterator ei = v1.edges_in.begin(); ei != v1.edges_in.end(); ei++)
            edges1.insert(perm[*ei]);
        edges2.clear();
        for (std::vector<unsigned int>::const_iterator ei = v2.edges_in.begin(); ei != v2.edges_in.end(); ei++)
            edges2.insert(*ei);
        if (!(edges1 == edges2))
            return false;

        edges1.clear();
        for (std::vector<unsigned int>::const_iterator ei = v1.edges_out.begin(); ei != v1.edges_out.end(); ei++)
            edges1.insert(perm[*ei]);
        edges2.clear();
        for (std::vector<unsigned int>::const_iterator ei = v2.edges_out.begin(); ei != v2.edges_out.end(); ei++)
            edges2.insert(*ei);
        if (!(edges1 == edges2))
            return false;
    }
...
bool Digraph::nucr_find_first_component(const unsigned int level, std::vector<unsigned int> &component,
    unsigned int &component_elements, Partition::Cell *&sh_return)
{
    component.clear();
    ...
    for (unsigned int i = 0; i < comp.size(); i++) {
        ...
        switch (sh) {
            case shs_f:
                ...
                break;
            case shs_fs:
                if (sh_return == 0 or cell->length < sh_size or (cell->length == sh_size and cell->first <= sh_first)) {
                    sh_return = cell;
                    sh_first = cell->first;
                    sh_size = cell->length;
                }
            ...
            case shs_fl:
                ...
                break;
            case shs_fm:
                if (sh_return == 0 or nuconn > sh_nuconn or (nuconn == sh_nuconn and cell->first <= sh_first)) {
                    sh_return = cell;
                    sh_first = cell->first;
                    sh_nuconn = nuconn;
                }
            ...
            case shs_fsm:
                if (sh_return == 0 or nuconn > sh_nuconn or
                    ...
                    (cell->length < sh_size or (cell->length == sh_size and cell->first <= sh_first)))) {
                    sh_return = cell;
                    sh_first = cell->first;
                    sh_size = cell->length;
                    sh_nuconn = nuconn;
...
void Graph::Vertex::remove_duplicate_edges(std::vector<bool> &tmp)
{
    ...
    for (unsigned int i = 0; i < tmp.size(); i++)
        assert(tmp[i] == false);
#endif
    for (std::vector<unsigned int>::iterator iter = edges.begin(); iter != edges.end();) {
        const unsigned int dest_vertex = *iter;
        if (tmp[dest_vertex] == true) {
            /* A duplicate edge found! */
    ...
    /* Clear tmp */
    for (std::vector<unsigned int>::iterator iter = edges.begin(); iter != edges.end(); iter++) {
        tmp[*iter] = false;
    }
#if defined(BLISS_CONSISTENCY_CHECKS)
    /* Post-conditions  */
 ...
 * Time complexity: O(1)
 */
unsigned int Graph::vertex_color_invariant(const Graph * const g, const unsigned int v)
{
 ...
 * Time complexity: O(1)
 */
unsigned int Graph::degree_invariant(const Graph * const g, const unsigned int v)
{
...
unsigned int Graph::selfloop_invariant(const Graph * const g, const unsigned int v)
{
    ...
    for (std::vector<unsigned int>::const_iterator ei = vertex.edges.begin(); ei != vertex.edges.end(); ei++) {
        if (*ei == v)
            return 1;
    }
    return 0;
...
bool Graph::refine_according_to_invariant(unsigned int (*inv)(const Graph * const g, const unsigned int v))
{
    bool refined = false;
...
bool Graph::is_equitable() const
{
    ...
    for (Partition::Cell *cell = p.first_cell; cell; cell = cell->next) {
        ...
        for (std::vector<unsigned int>::const_iterator ei = first_vertex.edges.begin(); ei != first_vertex.edges.end();
            ei++) {
            first_count[p.get_cell(*ei)->first]++;
        }
...
bool Graph::is_automorphism(unsigned int * const perm)
{
    ...
    for (unsigned int i = 0; i < get_nof_vertices(); i++) {
        Vertex &v1 = vertices[i];
        edges1.clear();
        for (std::vector<unsigned int>::iterator ei = v1.edges.begin(); ei != v1.edges.end(); ei++)
            edges1.insert(perm[*ei]);

        Vertex &v2 = vertices[perm[i]];
        edges2.clear();
        for (std::vector<unsigned int>::iterator ei = v2.edges.begin(); ei != v2.edges.end(); ei++)
            edges2.insert(*ei);
...
bool Graph::is_automorphism(const std::vector<unsigned int> &perm) const
{
    ...
    for (unsigned int i = 0; i < get_nof_vertices(); i++) {
        const Vertex &v1 = vertices[i];
        edges1.clear();
        for (std::vector<unsigned int>::const_iterator ei = v1.edges.begin(); ei != v1.edges.end(); ei++)
            edges1.insert(perm[*ei]);

        const Vertex &v2 = vertices[perm[i]];
        edges2.clear();
        for (std::vector<unsigned int>::const_iterator ei = v2.edges.begin(); ei != v2.edges.end(); ei++)
            edges2.insert(*ei);
...
bool Graph::nucr_find_first_component(const unsigned int level, std::vector<unsigned int> &component,
    unsigned int &component_elements, Partition::Cell *&sh_return)
{
    component.clear();
    component_elements = 0;
