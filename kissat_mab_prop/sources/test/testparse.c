...
static bool test_parse(bool expect_parse_error, unsigned strict, const char *path)
{
    const char *type;
    switch (strict) {
        default:
    ...
    tissat_verbose("Parsing %svalid '%s' in '%s' mode.", expect_parse_error ? "in" : "", path, type);
    ...
    uint64_t lineno;
    int max_var;
    const char *error = kissat_parse_dimacs(solver, strict, &file, &lineno, &max_var);
    if (expect_parse_error) {
        ...
        FATAL("%s parsing failed unexpectedly: %s:%" PRIu64 ": %s", type, path, lineno, error);
...
#define PARSE(STRICT, NAME)                                                                  \
    do {                                                                                     \
        ...
        if (STRICT == RELAXED_PARSING)                                                       \
            ...
            assert(STRICT == PEDANTIC_PARSING), expect_error = (strict == PEDANTIC_PARSING); \
        test_parse(expect_error, strict, path);                                              \
    } while (0)
    for (strictness strict = RELAXED_PARSING; strict <= PEDANTIC_PARSING; strict++) {
        PARSE(0, emptyfile);
