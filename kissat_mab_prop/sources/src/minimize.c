...
static inline bool minimize_reference(kissat *solver, assigned *assigned, reference ref, unsigned lit, int depth)
{
    const unsigned not_lit = NOT(lit);
    clause *c = kissat_dereference_clause(solver, ref);
    for (all_literals_in_clause(other, c))
        if (other != not_lit && !minimize_literal(solver, assigned, other, depth))
            return false;
    return true;
}

static inline bool minimize_binary(kissat *solver, assigned *assigned, unsigned lit, int depth)
{
    ...
    for (unsigned next = lit;;) {
        ...
        if (!a->binary) {
            res = minimize_reference(solver, assigned, a->reason, next, depth + 1);
            break;
        }
        next = a->reason;
    }
...
static bool minimize_literal(kissat *solver, assigned *assigned, unsigned lit, int depth)
{
    assert(VALUE(lit) < 0);
...
static void reset_minimize(kissat *solver)
{
