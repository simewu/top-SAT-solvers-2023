...
static inline void watch_hyper_delayed(kissat *solver, unsigneds *delayed)
{
    ...
    while (d != end_delayed) {
        ...
        if (watch.type.binary) {
            ...
            LOGBINARY(lit, watch.binary.lit, "watching blocking %s in %s", LOGLIT(lit), LOGLIT(watch.binary.lit));
            assert(lit < LITS);
...
static inline void delay_watching_hyper(kissat *solver, unsigneds *delayed, unsigned lit, unsigned other)
{
...
static inline clause *binary_propagate_literal(kissat *solver, unsigned lit)
{
    assert(solver->probing);
    ...
    while (p != end) {
        ...
        assert(!other_value);
        kissat_assign_binary(solver, values, assigned, redundant, other, not_lit);
    }
    return res;
}

static inline clause *hyper_propagate(kissat *solver, const clause *ignore)
{
    assert(solver->probing);
    assert(solver->watching);
    assert(solver->level == 1);
...
clause *kissat_hyper_propagate(kissat *solver, const clause *ignore)
{
    assert(solver->probing);
    assert(solver->watching);
