...
#define NREALLOC_GENERIC(TYPE, NAME, ELEMENTS_PER_BLOCK)                                                           \
    do {                                                                                                           \
        ...
        solver->NAME = kissat_nrealloc(solver, solver->NAME, old_size, new_size, ELEMENTS_PER_BLOCK * block_size); \
    } while (0)
...
#define CREALLOC_GENERIC(TYPE, NAME, ELEMENTS_PER_BLOCK)                                \
    do {                                                                                \
        const size_t block_size = sizeof(TYPE);                                         \
        TYPE *NAME = kissat_calloc(solver, ELEMENTS_PER_BLOCK * new_size, block_size);  \
        if (old_size) {                                                                 \
            const size_t bytes = ELEMENTS_PER_BLOCK * old_size * block_size;            \
            memcpy(NAME, solver->NAME, bytes);                                          \
        }                                                                               \
        kissat_dealloc(solver, solver->NAME, ELEMENTS_PER_BLOCK *old_size, block_size); \
        solver->NAME = NAME;                                                            \
    } while (0)

#define NREALLOC_VARIABLE_INDEXED(TYPE, NAME) NREALLOC_GENERIC(TYPE, NAME, 1)

#define NREALLOC_LITERAL_INDEXED(TYPE, NAME) NREALLOC_GENERIC(TYPE, NAME, 2)

#define CREALLOC_VARIABLE_INDEXED(TYPE, NAME) CREALLOC_GENERIC(TYPE, NAME, 1)

#define CREALLOC_LITERAL_INDEXED(TYPE, NAME) CREALLOC_GENERIC(TYPE, NAME, 2)
...
void kissat_increase_size(kissat *solver, unsigned new_size)
{
...
#ifndef NMETRICS
    LOG("%s before increasing size from %u to %u", FORMAT_BYTES(kissat_allocated(solver)), old_size, new_size);
#endif
    ...
    CREALLOC_VARIABLE_INDEXED(int, simplified_hints);
    // MAB
    ...
    if (solver->mode_mab)
        CREALLOC_VARIABLE_INDEXED(unsigned, mode_chosen);

    CREALLOC_VARIABLE_INDEXED(unsigneds, touched_list);
    CREALLOC_LITERAL_INDEXED(mark, marks);
    CREALLOC_LITERAL_INDEXED(value, values);
    CREALLOC_LITERAL_INDEXED(watches, watches);
    if (solver->PSIDS) {
        CREALLOC_LITERAL_INDEXED(double, p_scores);
    }
    {
...
#ifndef NMETRICS
    LOG("%s after increasing size from %zu to %zu", FORMAT_BYTES(kissat_allocated(solver)), old_size, new_size);
#endif
}
...
#ifndef NMETRICS
    LOG("%s before decreasing size from %u to %u", FORMAT_BYTES(kissat_allocated(solver)), old_size, new_size);
#endif
...
#ifndef NMETRICS
    LOG("%s after decreasing size from %zu to %zu", FORMAT_BYTES(kissat_allocated(solver)), old_size, new_size);
#endif
}
...
void kissat_enlarge_variables(kissat *solver, unsigned new_vars)
{
    ...
    if (old_size < new_vars) {
        LOG("old size %zu below requested new number of variables %u", old_size, new_vars);
        size_t new_size;
        if (!old_size)
            new_size = new_vars;
        else {
