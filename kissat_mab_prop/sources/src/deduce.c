...
static inline bool analyze_literal(kissat *solver, assigned *all_assigned, frame *frames, unsigned lit)
{
    assert(VALUE(lit) < 0);
...
clause *kissat_deduce_first_uip_clause(kissat *solver, clause *conflict)
{
    ...
    LOG("starting with %u unresolved literals on current decision level", unresolved_on_current_level);
    ...
    for (;;) {
        ...
        if (a->binary) {
            ...
            for (all_literals_in_clause(lit, reason))
                if (lit != uip && analyze_literal(solver, all_assigned, frames, lit))
                    unresolved_on_current_level++;
        ...
        LOGRES2("new");
#endif
        if (otfs && solver->antecedent_size > 2 && solver->resolvent_size < solver->antecedent_size) {
            assert(!a->binary);
