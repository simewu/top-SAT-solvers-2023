...
static clause *large_on_the_fly_strengthen(kissat *solver, clause *c, unsigned lit)
{
    ...
    }
    {
        ...
        LOGREF(ref, "updating watching %s now blocking %s in", LOGLIT(lits[0]), LOGLIT(lits[1]));
    }
#ifndef NDEBUG
...
static clause *binary_on_the_fly_strengthen(kissat *solver, clause *c, unsigned lit)
{
    assert(solver->antecedent_size == 3);
    LOGCLS(c,
    ...
    clause *conflict = kissat_binary_conflict(solver, redundant, first, second);
    INC(conflicts);
    return conflict;
}
