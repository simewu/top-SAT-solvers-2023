...
void kissat_check_shrink_clause(struct kissat *, struct clause *, unsigned remove, unsigned keep);
...
#define CHECK_AND_ADD_STACK(S) CHECK_AND_ADD_LITS(SIZE_STACK(S), BEGIN_STACK(S))
...
#define REMOVE_CHECKER_STACK(S)                                                    \
    do {                                                                           \
        if (GET_OPTION(check) > 1)                                                 \
            kissat_remove_checker_internal(solver, SIZE_STACK(S), BEGIN_STACK(S)); \
    } while (0)
