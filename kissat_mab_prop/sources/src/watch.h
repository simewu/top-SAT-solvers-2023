...
static inline watch kissat_binary_watch(unsigned lit, bool redundant, bool hyper)
{
...
#define LAST_WATCH_POINTER(WS) (watch *)kissat_last_vector_pointer(&solver->vectors, &WS)

#define BEGIN_WATCHES(WS) ((union watch *)kissat_begin_vector(&solver->vectors, &(WS)))

#define END_WATCHES(WS) ((union watch *)kissat_end_vector(&solver->vectors, &(WS)))

#define RELEASE_WATCHES(WS) kissat_release_vector(solver, &solver->vectors, &(WS))
...
#define REMOVE_WATCHES(W, E) kissat_remove_from_vector(solver, &solver->vectors, &(W), (E).raw)

#define WATCHES(LIT) (solver->watches[assert((LIT) < LITS), (LIT)])

#define all_binary_blocking_watch_ref(WATCH, REF, WATCHES)                                                  \
    watch WATCH, *WATCH##_PTR = (assert(solver->watching), BEGIN_WATCHES(WATCHES)),                         \
                 *WATCH##_END = END_WATCHES(WATCHES);                                                       \
    WATCH##_PTR != WATCH##_END &&                                                                           \
        ((WATCH = *WATCH##_PTR), (REF = WATCH.type.binary ? INVALID_REF : WATCH##_PTR[1].large.ref), true); \
