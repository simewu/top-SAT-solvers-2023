...
static inline value move_smallest_literal_to_front(kissat *solver, const value *values, const assigned *assigned,
    bool satisfied_is_enough, unsigned start, unsigned size, unsigned *lits)
{
    assert(1 < size);
    assert(start < size);
    ...
    LOG("new smallest literal %s at %u swapped with %s at %u", LOGLIT(best), pos, LOGLIT(a), start);
#ifndef LOGGING
    (void)solver;
#endif
    return u;
...
#endif
    value u = move_smallest_literal_to_front(solver, values, assigned, false, 0, size, lits);
    if (size > 2)
        move_smallest_literal_to_front(solver, values, assigned, (u >= 0), 1, size, lits);
}
