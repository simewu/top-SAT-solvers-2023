...
#define LOGIMPORTED3(...) LOGUNSIGNEDS3(SIZE_STACK(checker->imported), BEGIN_STACK(checker->imported), __VA_ARGS__)

#define LOGLINE3(...) LOGUNSIGNEDS3(line->size, line->lits, __VA_ARGS__)

#define MAX_NONCES (sizeof checker->nonces / sizeof *checker->nonces)

static inline bool less_unsigned(unsigned a, unsigned b)
{
    return a < b;
}
...
static void release_watches(kissat *solver, checker *checker)
{
    ...
    kissat_dealloc(solver, checker->watches, 2 * checker->size, sizeof(lines));
}
...
void kissat_print_checker_statistics(kissat *solver, bool verbose)
{
    ...
    PRINT_STAT("checker_checked", checker->checked, PERCENT_ADDED(checked), "%", "added");
    if (verbose)
        PRINT_STAT("checker_collisions", checker->collisions, kissat_percent(checker->collisions, checker->searches),
            "%", "per search");
    PRINT_STAT("checker_decisions", checker->decisions, kissat_average(checker->decisions, checker->checked), "",
        "per check");
    PRINT_STAT("checker_propagations", checker->propagations, kissat_average(checker->propagations, checker->checked),
        "", "per check");
    PRINT_STAT("checker_removed", checker->removed, PERCENT_ADDED(removed), "%", "added");
    PRINT_STAT("checker_unchecked", checker->unchecked, PERCENT_ADDED(unchecked), "%", "added");
}
...
static line *new_line(kissat *solver, checker *checker, unsigned size, unsigned hash)
{
    line *res = kissat_malloc(solver, bytes_line(size));
    res->next = 0;
    res->size = size;
    res->hash = hash;
    memcpy(res->lits, BEGIN_STACK(checker->imported), size * sizeof *res->lits);
    return res;
}
...
static void checker_assign(kissat *solver, checker *checker, unsigned lit, line *line)
{
#ifdef LOGGING
...
static lines *checker_watches(checker *checker, unsigned lit)
{
    assert(VALID_CHECKER_LIT(lit));
    return checker->watches + lit;
}

static void watch_checker_literal(kissat *solver, checker *checker, line *line, unsigned lit)
{
...
static void unwatch_checker_literal(kissat *solver, checker *checker, line *line, unsigned lit)
{
...
static void unwatch_line(kissat *solver, checker *checker, line *line)
{
    assert(line->size > 1);
...
static bool satisfied_or_trivial_imported(kissat *solver, checker *checker)
{
...
static bool simplify_imported(kissat *solver, checker *checker)
{
    ...
    for (p = lits; !res && p != end_of_lits; p++) {
        ...
        const unsigned not_lit = lit ^ 1;
        if (marks[not_lit]) {
            LOG3("simplified checker clause trivial (contains %u and %u)", not_lit, lit);
            res = true;
        } else {
...
static void insert_imported(kissat *solver, checker *checker, unsigned hash)
{
...
static void insert_imported_if_not_simplified(kissat *solver, checker *checker)
{
...
static bool match_line(checker *checker, unsigned size, unsigned hash, line *line)
{
    if (line->size != size)
        return false;
    if (line->hash != hash)
...
static void resize_checker(kissat *solver, checker *checker, unsigned new_vars)
{
    ...
    if (new_vars > size) {
        ...
        checker->marks = kissat_realloc(solver, checker->marks, size2, new_size2 * sizeof(bool));
        checker->values = kissat_realloc(solver, checker->values, size2, new_size2);
        checker->watches = kissat_realloc(solver, checker->watches, size2 * sizeof *checker->watches,
    ...
    if (delta == 1)
        ...
        LOG3("initializing %u checker variables from %u to %u", delta, vars, new_vars - 1);
...
static inline unsigned import_external_checker(kissat *solver, checker *checker, int elit)
{
    assert(elit);
...
static inline unsigned import_internal_checker(kissat *solver, checker *checker, unsigned ilit)
{
...
static line *find_line(kissat *solver, checker *checker, size_t size, bool remove)
{
    if (!checker->hashed)
        return 0;
    sort_line(solver, checker);
    ...
    const unsigned reduced = reduce_hash(hash, checker->hashed);
    struct line **p, *line;
    for (p = checker->table + reduced; (line = *p) && !match_line(checker, size, hash, line); p = &line->next)
        checker->collisions++;
    if (checker->marked)
        unmark_line(checker);
    if (line && remove)
...
static void import_external_literals(kissat *solver, checker *checker, size_t size, int *elits)
{
    if (size > UINT_MAX)
        kissat_fatal("can not check handle original clause of size %zu", size);
    CLEAR_STACK(checker->imported);
    for (size_t i = 0; i < size; i++) {
        const unsigned lit = import_external_checker(solver, checker, elits[i]);
        PUSH_STACK(checker->imported, lit);
    }
    LOGIMPORTED3("checker imported external");
}

static void import_internal_literals(kissat *solver, checker *checker, size_t size, const unsigned *ilits)
{
    assert(size <= UINT_MAX);
    CLEAR_STACK(checker->imported);
    for (size_t i = 0; i < size; i++) {
...
static void import_binary(kissat *solver, checker *checker, unsigned a, unsigned b)
{
    CLEAR_STACK(checker->imported);
...
static void import_internal_unit(kissat *solver, checker *checker, unsigned a)
{
    CLEAR_STACK(checker->imported);
...
static bool line_redundant(kissat *solver, checker *checker, size_t size)
{
    ...
    if (size == 1) {
        ...
        if (value < 0 && !checker->inconsistent)
            kissat_fatal("consistent checker can not remove falsified unit %d", export_checker(checker, unit));
        if (!value)
            kissat_fatal("checker can not remove unassigned unit %d", export_checker(checker, unit));
...
static void remove_line_if_not_redundant(kissat *solver, checker *checker)
{
    size_t size = SIZE_STACK(checker->imported);
...
void kissat_add_unchecked_external(kissat *solver, size_t size, int *elits)
{
...
void kissat_add_unchecked_internal(kissat *solver, size_t size, unsigned *lits)
{
...
void kissat_check_and_add_internal(kissat *solver, size_t size, unsigned *lits)
{
...
void kissat_check_shrink_clause(kissat *solver, clause *c, unsigned remove, unsigned keep)
{
...
void kissat_remove_checker_external(kissat *solver, size_t size, int *elits)
{
...
void kissat_remove_checker_internal(kissat *solver, size_t size, unsigned *ilits)
{
