...
#define D 4
#define CHILD(POS) (D * (POS) + 1)
#define LAST_CHILD(POS) (D * (POS) + D)
#define PARENT(POS) (((POS)-1) / D)
...
void kissat_check_heap(heap *heap)
{
    ...
    for (unsigned i = 0; i < end; i++) {
        ...
        unsigned last_child = LAST_CHILD(idx_pos);
        while (child_pos < end && child_pos <= last_child) {
...
void kissat_resize_heap(kissat *solver, heap *heap, unsigned new_size)
{
    ...
    if (old_size >= new_size)
        return;
    LOG("resizing %s heap from %u to %u", (heap->tainted ? "tainted" : "untainted"), old_size, new_size);

    heap->pos = kissat_nrealloc(solver, heap->pos, old_size, new_size, sizeof(unsigned));
    if (heap->tainted) {
        heap->score = kissat_nrealloc(solver, heap->score, old_size, new_size, sizeof(double));
    } else {
        if (old_size)
...
static void bubble_up(kissat *solver, heap *heap, unsigned idx)
{
    ...
    while (idx_pos) {
        ...
        if (score[parent] >= idx_score)
            break;
        LOG("heap bubble up: %u@%u = %g swapped with %u@%u = %g", parent, parent_pos, score[parent], idx, idx_pos,
            idx_score);
...
inline static int get_max_child(heap *heap, int bg, int len)
{
    const unsigned *stack = BEGIN_STACK(heap->stack);
    const double *score = heap->score;
    switch (len) {
        case 1:
            return bg;
        case 2:
            return bg + (score[stack[bg + 1]] > score[stack[bg]]);
        case 3: {
            int i = (score[stack[bg + 1]] > score[stack[bg]]);
            return (score[stack[bg + 2]] > score[stack[bg + i]]) ? bg + 2 : bg + i;
        }
        case 4: {
            int i = (score[stack[bg + 1]] > score[stack[bg]]);
            int j = (score[stack[bg + 3]] > score[stack[bg + 2]]);
            return (score[stack[bg + j + 2]] > score[stack[bg + i]]) ? bg + j + 2 : bg + i;
        }
        default: {
            return -123456;
        }
...
static void bubble_down(kissat *solver, heap *heap, unsigned idx)
{
    unsigned *stack = BEGIN_STACK(heap->stack);
    const unsigned end = SIZE_STACK(heap->stack);
    unsigned *pos = heap->pos;
    ...
    for (;;) {
        ...
        if (child_pos >= end)
            break;
        unsigned len = min(end, LAST_CHILD(idx_pos) + 1) - child_pos;
        child_pos = get_max_child(heap, child_pos, len);
        unsigned child = stack[child_pos];
        double child_score = score[child];
        if (child_score <= idx_score)
            break;
        LOG("heap bubble down: %u@%u = %g swapped with %u@%u = %g", child, child_pos, score[child], idx, idx_pos,
            idx_score);
        stack[idx_pos] = child;
        pos[child] = idx_pos;
        idx_pos = child_pos;
...
static void enlarge_heap(kissat *solver, heap *heap, unsigned new_vars)
{
...
void kissat_update_heap(kissat *solver, heap *heap, unsigned idx, double new_score)
{
...
double kissat_max_score_on_heap(heap *heap)
{
    if (!heap->tainted)
        return 0;
    assert(heap->vars);
