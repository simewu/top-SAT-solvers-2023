...
void kissat_reset_target_assigned(kissat *solver)
{
    ...
    solver->target_assigned = 0.9 * solver->target_assigned;
}
...
static char rephase_flipped(kissat *solver)
{
    for (all_phases(p))
        p->saved *= -1;
    return 'F';
...
static char rephase_random(kissat *solver)
{
    for (all_phases(p))
...
static char rephase_greedy(kissat * solver) {
    for (all_phases(p)) {
        p->saved = p->greedy;
    }
    return 'G';
...
static void reset_phases(kissat *solver)
{
    ...
    if (!count)
        ...
        if (solver->PSIDS) {
            int choice = (count - 2) % phase_count;
            if (choice == 1 || choice == 4 || choice == 7 || choice == 10) {
                count++;
            }
        }
        switch ((count - 2) % phase_count) {
            default:
            case 0:
    ...
    switch (type) {
        ...
        case 'W':
            ...
            break;
        case 'G':
            type_as_string = "greedy";
            break;
    }
    assert(type_as_string);
    kissat_phase(solver, "rephase", GET(rephased), "%s phases in %s search mode", type_as_string,
