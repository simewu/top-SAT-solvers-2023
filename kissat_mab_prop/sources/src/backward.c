...
static bool backward_subsume_lits(kissat *solver, reference ignore, unsigned size, unsigned *lits)
{
    assert(size > 1);
    ...
    LOGTMP("backward subsumption on %s with %zu occurrences in", LOGLIT(min_lit), min_occs);
    ...
    const unsigned match = (size > 2 ? INVALID_LIT : (lits[0] ^ lits[1] ^ min_lit));
        ...
        if (watch.type.binary) {
            ...
            if (other == not_match) {
                LOGBINARY(min_lit, match,
                    ...
                    "on %s first antecedent",
                    LOGLIT(match));
                LOGBINARY(min_lit, not_match,
                    "backward hyper unary resolution"
                    "on %s second antecedent",
                    LOGLIT(not_match));
                unit = true;
                break;
            }
            ...
            LOGCLS(c, "backward strengthening by removing %s in", LOGLIT(remove));
            ...
            if (old_size == 2) {
                LOGTMP("backward large hyper unary resolution on %s"
                    ...
                    LOGLIT(NOT(remove)));
                LOGCLS(c,
                    "backward large hyper unary resolution on %s"
                    "second antecedent",
                    LOGLIT(remove));
                unit = true;
                break;
