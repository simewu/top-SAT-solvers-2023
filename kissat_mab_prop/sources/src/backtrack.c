...
static inline void add_unassigned_variable_back_to_queue(kissat *solver, links *links, unsigned lit)
{
...
static inline void add_unassigned_variable_back_to_heap(kissat *solver, heap *scores, unsigned lit)
{
    assert(solver->stable);
    const unsigned idx = IDX(lit);
...
static void update_phases(kissat *solver)
{
    ...
    if (!solver->probing) {
        ...
        if (solver->target_assigned < solver->consistently_assigned) {
            LOG("updating target assigned from %u to %u", solver->target_assigned, solver->consistently_assigned);
        ...
        if (solver->best_assigned < solver->consistently_assigned) {
            LOG("updating best assigned from %u to %u", solver->best_assigned, solver->consistently_assigned);
