...
#define PEEK_STACK(S, N) (BEGIN_STACK(S)[assert((N) < SIZE_STACK(S)), (N)])
...
typedef STACK(ints) intss;
typedef STACK(size_t) sizes;
typedef STACK(unsigned) unsigneds;
typedef STACK(unsigneds) unsignedss;
typedef STACK(long long) longlongs;
// *INDENT-ON*
