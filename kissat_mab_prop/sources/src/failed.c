...
static bool occurs_in_binary_clause(kissat *solver, const value *values, unsigned lit)
{
...
static size_t schedule_probes(kissat *solver, unsigned round, unsigneds *roots)
{
    assert(!solver->level);
    ...
    for (all_variables(idx)) {
        ...
        if (pos && neg) {
            LOG("skipping variable %u occurring positively and negatively", idx);
            continue;
        }
        if (!pos && !neg) {
    ...
    kissat_phase(solver, "failed", GET(failed_literal_probings),
        "round %u: scheduled %zu probes %.0f%% out of all variables", round, res, kissat_percent(res, solver->active));
    if (scheduled[1])
        kissat_phase(solver, "failed", GET(failed_literal_probings),
            "round %u: prioritized %u probes %.0f%% of all scheduled", round, scheduled[1],
            kissat_percent(scheduled[1], res));
#endif
    return res;
}

static inline bool less_stable_probe(kissat *solver, const flags *flags, const heap *scores, unsigned a, unsigned b)
{
#ifdef NDEBUG
    (void)solver;
#endif
...
static inline unsigned less_focused_probe(kissat *solver, const flags *flags, const links *links, unsigned a,
    unsigned b)
{
#ifdef NDEBUG
    (void)solver;
...
#define LESS_STABLE_PROBE(A, B) less_stable_probe(solver, flags, scores, (A), (B))

#define LESS_FOCUSED_PROBE(A, B) less_focused_probe(solver, flags, links, (A), (B))
...
static bool probe_round(kissat *solver, unsigned round, uint64_t limit, unsigneds *probes, unsigned *failed_ptr,
    uint64_t *resolved_ptr)
{
    ...
    while (!EMPTY_STACK(*probes)) {
        ...
        if (conflict) {
            ...
            if (!values[probe]) {
                ...
                LOG("learned unit %s from failed literal %s", LOGLIT(unit), LOGLIT(probe));
        ...
        if (!prioritized) {
            kissat_phase(solver, "failed", GET(failed_literal_probings), "round %u: prioritizing remaining %zu probes",
                round, remain);
            while (!EMPTY_STACK(*probes)) {
            ...
            kissat_phase(solver, "failed", GET(failed_literal_probings),
                "round %u: keeping %zu probes %.0f%% prioritized", round, prioritized,
    ...
    kissat_phase(solver, "failed", GET(failed_literal_probings),
        ...
        round, failed, kissat_percent(failed, probed), probed, kissat_percent(probed, scheduled));

    kissat_phase(solver, "failed", GET(failed_literal_probings),
        "round %u: %" PRIu64 " hyper binary resolvents "
        "%.1f per probe",
        round, redundant, kissat_average(redundant, probed));
#else
    (void)round;
#endif
    ...
    *failed_ptr = failed;
    ...
    if (solver->inconsistent) {
        kissat_phase(solver, "failed", GET(failed_literal_probings), "last round produced empty clause");
        return false;
    }

    if (!failed)
        kissat_phase(solver, "failed", GET(failed_literal_probings), "no failed literal produced in last round");
    if (!redundant)
        kissat_phase(solver, "failed", GET(failed_literal_probings),
...
void kissat_failed_literal_probing(kissat *solver)
{
    if (solver->inconsistent)
        return;
    ...
    SET_EFFICIENCY_BOUND(limit, failed, probing_ticks, search_ticks, kissat_nlogn(solver->active));
    ...
    for (;;) {
        ...
        if (!scheduled) {
            kissat_phase(solver, "failed", GET(failed_literal_probings), "no roots found in binary implication graph");
            break;
        }
        ...
        if (!EMPTY_STACK(roots)) {
            ...
            kissat_phase(solver, "failed", GET(failed_literal_probings), "incomplete probing with %zu remaining %.0f%%",
                remain, kissat_percent(remain, scheduled));
#endif
            break;
        }
        if (round == max_rounds) {
            kissat_phase(solver, "failed", GET(failed_literal_probings), "completed probing until maximum round");
            break;
        }
        round++;
    }
    ...
    if (failed)
        kissat_phase(solver, "failed", GET(failed_literal_probings), "%u failed literals in total in %u rounds", failed,
            round);
    else
        kissat_phase(solver, "failed", GET(failed_literal_probings), "no failed literals in %u rounds", round);

    if (resolved)
        kissat_phase(solver, "failed", GET(failed_literal_probings),
            "%" PRIu64 " hyper binary resolvents in total in %u rounds", resolved, round);
    else
        kissat_phase(solver, "failed", GET(failed_literal_probings), "no hyper binary resolvents in total in %u rounds",
            round);
