...
static bool really_substitute(kissat *solver, bool first)
{
        ...
        while (!solver->inconsistent && !EMPTY_STACK(work)) {
            ...
            if (lit == INVALID_LIT) {
                ...
                if (size_scc > 1) {
                    ...
                    non_trivial_sccs++;
                    LOG("size %u SCC entered trough %s representative %s", size_scc, LOGLIT(lit), LOGLIT(min_lit));
                } else {
                    trivial_sccs++;
                ...
                for (const unsigned *p = begin_scc; p != end_scc; p++) {
                    ...
                    if (min_lit == repr_not_other) {
                        LOG("clashing literals %s and %s in same SCC", LOGLIT(other), LOGLIT(not_other));
                    ...
                    if (mark_root > mark_not_other)
                        continue;
                    LOG("root %s implies both %s and %s", LOGLIT(root), LOGLIT(other), LOGLIT(not_other));
...
static void remove_representative_equivalences(kissat *solver, unsigned *repr, bool *eliminate)
{
    if (!solver->inconsistent) {
    ...
    for (all_literals(lit)) {
        ...
        while (p != end) {
            ...
            if (repr_other == not_repr_lit) {
                ...
                if (lit < other) {
                    removed++;
                    kissat_delete_binary(solver, src.binary.redundant, src.binary.hyper, lit, other);
                }
...
static void substitute_clauses(kissat *solver, unsigned *repr)
{
    ...
    for (all_clauses(c)) {
        ...
        for (all_literals_in_clause(lit, c)) {
            ...
            if (repr_value < 0) {
                LOG("dropping falsified substituted %s (was %s)", LOGLIT(repr_lit), LOGLIT(lit));
                shrink = true;
                continue;
            }
            if (repr_value > 0) {
                LOGCLS(c, "satisfied by substituted %s (was %s)", LOGLIT(repr_lit), LOGLIT(lit));
                satisfied = true;
                break;
            }
            if (lit != repr_lit) {
                assert(!values[repr_lit]);
                LOG("substituted literal %s (was %s)", LOGLIT(repr_lit), LOGLIT(lit));
                substitute = true;
            } else
        ...
        if (satisfied || tautological) {
            ...
            if (!size) {
                ...
                LOGBINARY(first, second, "substituted %s", redundant ? "redundant" : "irredundant");
...
static bool substitute_round(kissat *solver, unsigned round)
{
    ...
    kissat_phase(solver, "substitute", GET(substitutions), "round %u removed %u variables %.0f%%", round, removed,
...
void kissat_substitute(kissat *solver, bool first)
{
    if (solver->inconsistent)
        return;
