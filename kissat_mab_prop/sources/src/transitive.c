...
static bool transitive_reduce(kissat *solver, unsigned src, uint64_t limit, uint64_t *reduced_ptr, unsigned *units)
{
    bool res = false;
    assert(!VALUE(src));
    ...
    for (watch *p = begin_src; p != end_src; p++) {
        ...
        while (!transitive && !failed && solver->propagated < SIZE_STACK(solver->trail)) {
            ...
            for (const watch *q = begin_lit; q != end_lit; q++) {
                ...
                if (value < 0) {
                    LOG("both %s and %s reachable from %s", LOGLIT(NOT(other)), LOGLIT(other), LOGLIT(src));
                    failed = true;
                    break;
                }
                if (!value)
        ...
        if (transitive) {
            ...
            kissat_delete_binary(solver, src_watch.binary.redundant, src_watch.binary.hyper, src, dst);
...
void kissat_transitive_reduction(kissat *solver)
{
    ...
    kissat_phase(solver, "transitive", GET(probings), "probed %u (%.0f%%): reduced %" PRIu64 ", units %u", probed,
