...
void init_cdata (kissat * solver)
{
  solver->cdata.succ_no_confs = 0; 
  solver->cdata.num_cdp = 0; 
  solver->cdata.confs_last_decision = 0;
  solver->cdata.total_cdp_len = 0;
  solver->cdata.decisions_with_confs = 0;
  solver->cdata.decisions_without_confs = 0;
  ...
  solver->cdata.last_decision = 0;
  solver->cdata.action_applied_amid_cd = 0;
  solver->cdata.decide_rand = 0;
  solver->cdata.rand_select_start = 0;
  solver->cdata.exploration_depth_bound = 10;

  solver->cdata.rand_dec_confs = 0;
  solver->cdata.rand_dec_learn_glue = 0;
  solver->cdata.rand_dec_learn_size = 0;
  solver->total_glue = 0; 
  solver->total_size = 0;
  solver->rand_selection = 0;
  solver->total_learnt = 0;
  solver->cdata.decide_rand_success = 0;
  solver->cdata.decide_rand_success_tries = 0;
  srand (42);
}


kissat *kissat_init(int status) {
    ...
    init_cdata (solver);
    if(status == 40) {
        solver->unsat_but_no_proof = true;
    }else {
        solver->unsat_but_no_proof = false;
    }
#ifndef NDEBUG
...
{
    // RELEASE_HEAP(solver->simplfied_units);
    DEALLOC_VARIABLE_INDEXED(simplified_hints);
    kissat_require_initialized(solver);

    kissat_release_heap(solver, &solver->scores);
    DEALLOC_VARIABLE_INDEXED(touched_list);
    if (solver->heuristic == 1 || solver->mab)
        DEALLOC_VARIABLE_INDEXED(conflicted_chb);
    if (solver->mab)
        DEALLOC_VARIABLE_INDEXED(mab_chosen);
    if (solver->mode_mab)
        DEALLOC_VARIABLE_INDEXED(mode_chosen);
    
    // GENE
    /*for (unsigned p = 0; p < SIZE_STACK(solver->seeds); p++) {
        value *s = PEEK_STACK(solver->seeds, p);
        kissat_free(solver, s, VARS * sizeof(value));
    }*/

    if (solver->PSIDS) {
        DEALLOC_LITERAL_INDEXED(p_scores);
    }
...
{
    kissat_require_initialized(solver);
    kissat_require(0 <= max_var, "negative maximum variable argument '%d'", max_var);
    kissat_require(max_var <= EXTERNAL_MAX_VAR, "invalid maximum variable argument '%d'", max_var);
...
void kissat_set_decision_limit(kissat *solver, unsigned limit)
{
    ...
    LOG("set decision limit to %" PRIu64 " after %u decisions", limits->decisions, limit);
}
...
void kissat_set_conflict_limit(kissat *solver, unsigned limit)
{
    ...
    LOG("set conflict limit to %" PRIu64 " after %u conflicts", limits->conflicts, limit);
}
...
void kissat_add(kissat *solver, int elit)
{
    ...
    if (elit) {
        ...
        if (!mark) {
            ...
            if (value > 0) {
                if (!solver->clause.satisfied) {
                    LOG("adding root level satisfied literal %u(%d)@0=1", ilit, elit);
            ...
            if (!solver->clause.trivial) {
                LOG("adding dual literal %u(%d) and %u(%d)", NOT(ilit), -elit, ilit, elit);
        ...
        if (checking) {
            ...
            solver->offset_of_last_original_clause = SIZE_STACK(solver->original);
