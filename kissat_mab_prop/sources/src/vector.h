...
static inline unsigned *kissat_begin_vector(vectors *vectors, vector *vector)
{
    return BEGIN_STACK(vectors->stack) + vector->offset;
}

static inline unsigned *kissat_end_vector(vectors *vectors, vector *vector)
{
    return kissat_begin_vector(vectors, vector) + vector->size;
}

static inline void kissat_inc_usable(vectors *vectors)
{
    assert(MAX_SECTOR > vectors->usable);
    vectors->usable++;
}

static inline void kissat_add_usable(vectors *vectors, sector inc)
{
    assert(MAX_SECTOR - inc >= vectors->usable);
    vectors->usable += inc;
}

static inline unsigned *kissat_last_vector_pointer(vectors *vectors, vector *vector)
{
    assert(vector->size);
...
static inline void kissat_pop_vector(vectors *vectors, vector *vector)
{
...
void kissat_remove_from_vector(struct kissat *, vectors *, vector *, unsigned);
void kissat_resize_vector(struct kissat *, vectors *, vector *, sector);
void kissat_release_vector(struct kissat *, vectors *, vector *);

static inline void kissat_dec_usable(vectors *vectors)
{
    assert(vectors->usable > 0);
    vectors->usable--;
}

static inline void kissat_push_vectors(struct kissat *solver, vectors *vectors, vector *vector, unsigned e)
{
...
#define all_vector(E, V, VS)                                                     \
    unsigned E, *E_PTR = kissat_begin_vector(VS, &V), *E_END = E_PTR + (V).size; \
