...
void init_app(application *application, kissat *solver)
{
...
#define LONG_FALSE_OPTION(ARG, NAME) \
    (!strcmp((ARG), "--no-" NAME) || !strcmp((ARG), "--" NAME "=0") || !strcmp((ARG), "--" NAME "=false"))

#endif

#define LONG_TRUE_OPTION(ARG, NAME) \
    (!strcmp((ARG), "--" NAME) || !strcmp((ARG), "--" NAME "=1") || !strcmp((ARG), "--" NAME "=true"))
...
bool parse_options(application *application, int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (single_first_option(arg))
            ERROR("option '%s' only allowed as %s argument", arg, i == 1 ? "single" : "first");
#ifndef NPROOFS
        else if (!strcmp(arg, "-f") || LONG_TRUE_OPTION(arg, "force") || LONG_TRUE_OPTION(arg, "forced")) {
            ...
            if (strict_option) {
                if (application->strict != RELAXED_PARSING)
                    ERROR("can not combine contradictory '%s' and '%s'", strict_option, arg);
            ...
            strict_option = arg;
        } else if (LONG_TRUE_OPTION(arg, "strict") || LONG_TRUE_OPTION(arg, "stricter") ||
...
#endif
        else if (!strcmp(arg, "--color") || !strcmp(arg, "--colors") || !strcmp(arg, "--colour") ||
            !strcmp(arg, "--colours"))
            kissat_force_colors();
        else if (!strcmp(arg, "--no-color") || !strcmp(arg, "--no-colors") || !strcmp(arg, "--no-colour") ||
            ...
            if (kissat_parse_option_value(valstr, &val) && val >= 0) {
                if (application->conflicts >= 0)
                    ERROR("multiple '--conflicts=%d' and '%s'", application->conflicts, arg);
            ...
            if (kissat_parse_option_value(valstr, &val) && val >= 0) {
                if (application->decisions >= 0)
                    ERROR("multiple '--decisions=%d' and '%s'", application->decisions, arg);
        ...
        else if (LONG_TRUE_OPTION(arg, "binary")) {
            application->binary = 1;
        }
#endif
#ifndef NOPTIONS
        else if (arg[0] == '-' && arg[1] == '-' && kissat_has_configuration(arg + 2)) {
            if (configuration)
...
#endif
#ifdef QUIET
        else if (arg[0] == '-' && !arg[2] && (arg[1] == 'q' || arg[1] == 's' || arg[1] == 'v'))
...
#endif
#ifdef NOPTIONS
        else if (arg[0] == '-' && !arg[2] && (arg[1] == 'l' || arg[1] == 'q' || arg[1] == 's' || arg[1] == 'v'))
            ...
            ERROR("three file arguments '%s', '%s' and '%s' (try '-h')", application->input_path,
    ...
    solver->mode_mab = GET_OPTION(modemab);
    if (solver->mab) {
        for (unsigned i = 0; i < solver->mab_heuristics; i++) {
            solver->mab_reward[i] = 0;
            solver->mab_select[i] = 0;
            // printf("mab time uninitialized: %d %g\n", i, solver->mab_time[i]);
            solver->mab_time[i] = 0;
            solver->mab_propagations[i] = 0;
            solver->mab_ticks[i] = 0;
        }
        solver->mabc = GET_OPTION(mabcint) + 0.1 * GET_OPTION(mabcdecimal);
        solver->mab_select[solver->heuristic]++;
        solver->mab_tot_time = 0;
        solver->mab_last_time = 0;
        solver->mab_tot_propagations = 0;
        solver->mab_last_propagations = 0;
        solver->mab_last_ticks = 0;
        solver->mab_tot_ticks = 0;
    }
    if (solver->mode_mab) {
        solver->modec = GET_OPTION(modecint) + 0.1 * GET_OPTION(mabcdecimal);
    }
    //solver->max_seed_count = GET_OPTION(geneseedcount);
    //INIT_STACK(solver->seeds);
    return true;
}
...
static bool parse_input(application *application)
{
    ...
    kissat_message(solver, "opened and reading %sDIMACS file:", file.compressed ? "compressed " : "");
    ...
    bool res = kissat_simplify(solver, &application->max_var, &file);
    // printf("fgafsda\n");

    if (!res) {
        printf("c solved by simplify\n");
        return false;
    }
...
#ifndef QUIET
    kissat_message(solver, "closing input after reading %s", FORMAT_BYTES(file.bytes));
    if (file.compressed) {
        assert(path);
        size_t bytes = kissat_file_size(path);
        kissat_message(solver, "inflated input file of size %s by %.2f", FORMAT_BYTES(bytes),
            kissat_average(file.bytes, bytes));
    }
    kissat_message(solver, "finished parsing after %.2f seconds", kissat_process_time() - entered);
#endif
    return true;
}
...
static bool write_proof(application *application)
{
    ...
    bool binary = true; // binary == true: DRAT, binary == false: pseudo boolean
    if (!strcmp(path, "-")) {
        binary = false;
    ...
    kissat_message(solver, "%swriting proof to %spbp file:", file->close ? "opened and " : "",
...
static void close_proof(application *application, int res)
{
    const char *path = application->proof_path;
    if (!path)
        return;
    kissat_release_proof(application->solver, res);
...
static void print_option(kissat *solver, int value, const opt *o)
{
    ...
    kissat_message(solver, "--%-30s (%s default '%s')", buffer, (value == o->value ? "same as" : "different from"),
        def_str);
}
#endif
...
static void print_limits(application *application)
{
    kissat *solver = application->solver;
    const int verbosity = kissat_verbosity(solver);
    if (verbosity < 1 && application->conflicts < 0 && application->decisions < 0)
        return;

    kissat_section(solver, "limits");
    if (!application->time && application->conflicts < 0 && application->conflicts < 0)
        kissat_message(solver, "no time, conflict nor decision limit set");
    else {
        if (application->time)
            kissat_message(solver, "time limit set to %d seconds", application->time);
        else if (verbosity > 0)
            kissat_message(solver, "no time limit");

        if (application->conflicts >= 0)
            kissat_message(solver, "conflict limit set to %d conflicts", application->conflicts);
        else if (verbosity > 0)
            kissat_message(solver, "no conflict limit");

        if (application->decisions >= 0)
            kissat_message(solver, "decision limit set to %d decisions", application->decisions);
        else if (verbosity > 0)
...
int kissat_application(kissat *solver, int argc, char **argv)
{
    ...
    if (res) {
        ...
        if (res == 20) {
            if(solver->symmetry_broken_without_proof) {
                res = 40;
            }else {
                printf("s UNSATISFIABLE\n");
                fflush(stdout);
            ...
            kissat_complete_val(solver);
            free(solver->last_val);
        }
    }
#ifndef QUIET
...
#endif
#ifndef NPROOFS
    close_proof(&application, res);
#endif
#ifndef QUIET
    kissat_section(solver, "shutting down");
    kissat_message(solver, "exit %d", res);
#endif
    //    kissat_message(solver, "exit time %g", kissat_process_time());
