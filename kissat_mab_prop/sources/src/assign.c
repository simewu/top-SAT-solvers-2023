...
static inline void kissat_assign(kissat *solver,
#ifdef INLINE_ASSIGN
    value *values, assigned *assigned,
#endif
    unsigned lit, bool binary, bool redundant, unsigned level, unsigned reason)
{
...
static inline unsigned kissat_assignment_level(kissat *solver, value *values, assigned *assigned, unsigned lit,
    clause *reason)
{
    unsigned res = 0;
    ...
    bool redundant, unsigned lit, unsigned other)
{
    ...
    LOGBINARY(lit, other, "assign %s %s reason", LOGLIT(lit), redundant ? "redundant" : "irredundant");
}
