...
static inline void update_after_adding_variable(kissat *solver, heap *schedule, unsigned idx)
{
...
static inline void update_after_removing_variable(kissat *solver, unsigned idx)
{
    assert(!solver->probing);
    heap *schedule = &solver->schedule;
    if (!schedule->size)
...
static inline void update_after_removing_clause(kissat *solver, clause *c, unsigned except)
{
    assert(c->garbage);
...
void kissat_update_after_removing_clause(kissat *solver, clause *c, unsigned except)
{
...
static size_t schedule_variables(kissat *solver)
{
    ...
    kissat_phase(solver, "eliminate", GET(eliminations), "scheduled %zu variables %.0f%%", scheduled,
...
#endif
    if (!kissat_dense_propagate(solver, NO_DENSE_PROPAGATION_LIMIT, INVALID_IDX)) {
        ...
        if (other == INVALID_LIT) {
            if (satisfied)
                ...
                if (!size) {
                    ...
                    if (backward)
                        backward = kissat_backward_subsume_temporary(solver, ref);
                }
            }
...
static void set_next_elimination_bound(kissat *solver, bool complete)
{
    ...
    if (complete) {
        if (current_bound == max_bound) {
            kissat_phase(solver, "eliminate", GET(eliminations), "completed maximum elimination bound %u",
                current_bound);
            limits *limits = &solver->limits;
            statistics *statistics = &solver->statistics;
            ...
            kissat_phase(solver, "eliminate", GET(eliminations), "completed elimination bound %u next %u",
        ...
        kissat_phase(solver, "eliminate", GET(eliminations), "incomplete elimination bound %u", current_bound);
}
...
{
    kissat_very_verbose(solver, "trying to eliminate variables with bound %u",
        solver->bounds.eliminate.additional_clauses);

    unsigned eliminated = 0;
    const unsigned active = solver->active;

    SET_EFFICIENCY_BOUND(resolution_limit, eliminate, resolutions, search_ticks, 2 * CLAUSES + kissat_nlogn(active));
    ...
    kissat_phase(solver, "eliminate", GET(eliminations), "eliminated %u variables %.0f%% out of %u in %zu rounds",
        eliminated, kissat_percent(eliminated, active), active, round);
    if (!solver->inconsistent)
...
{
    solver->bounds.eliminate.clause_size = SCALE_LIMIT(eliminations, eliminateclslim);

    solver->bounds.eliminate.occurrences = SCALE_LIMIT(eliminations, eliminateocclim);

    kissat_phase(solver, "eliminate", GET(eliminations), "occurrence limit %u and clause limit %u",
        solver->bounds.eliminate.occurrences, solver->bounds.eliminate.clause_size);

    solver->bounds.subsume.clause_size = SCALE_LIMIT(eliminations, subsumeclslim);

    solver->bounds.subsume.occurrences = SCALE_LIMIT(eliminations, subsumeocclim);

    kissat_phase(solver, "subsume", GET(eliminations), "occurrence limit %u clause limit %u",
        solver->bounds.subsume.occurrences, solver->bounds.subsume.clause_size);

    solver->bounds.xor.clause_size = log(MAX(solver->bounds.eliminate.occurrences, 1)) / log(2.0) + 0.5;
...
static void eliminate(kissat *solver)
{
    ...
    kissat_phase(solver, "eliminate", GET(eliminations), "elimination limit of %" PRIu64 " conflicts hit",
