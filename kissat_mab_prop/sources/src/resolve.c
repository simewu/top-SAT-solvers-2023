...
static unsigned occurrences_literal(kissat *solver, unsigned lit, bool *update)
{
    ...
    while (p != end) {
        ...
        if (head.type.binary) {
            ...
            if (c->garbage)
                ...
                LOG("literal %s watches too long clause of size %u", LOGLIT(lit), c->size);
                failed = true;
                break;
            } else
                res++;
...
static inline clause *watch_to_clause(kissat *solver, const value *values, const word *arena, clause *tmp, unsigned lit,
    watch watch)
{
    clause *res;
...
static bool generate_resolvents(kissat *solver, unsigned lit, statches *watches0, statches *watches1,
    uint64_t *resolved_ptr, uint64_t limit)
{
    ...
    for (all_stack(watch, watch0, *watches0)) {
        ...
        for (all_stack(watch, watch1, *watches1)) {
            clause *d = watch_to_clause(solver, values, arena, &tmp1, not_lit, watch1);
            ...
            LOGLITS(size_resolvent, BEGIN_STACK(solver->resolvents) + saved, "resolvent");
...
bool kissat_generate_resolvents(kissat *solver, unsigned idx, unsigned *lit_ptr)
{
    ...
    if (gates) {
        ...
        if (!generate_resolvents(solver, lit, gates0, antecedents1, &resolved, limit))
            failed = true;
        else {
            LOG("resolving gates[1] against antecedents[0] clauses");
            if (!generate_resolvents(solver, not_lit, gates1, antecedents0, &resolved, limit))
                failed = true;
            else if (solver->resolve_gate) {
                LOG("need to resolved gates[0] against gates[1] too");
                if (!generate_resolvents(solver, lit, gates0, gates1, &resolved, limit))
                    failed = true;
            }
        }
    } else {
        LOG("no gate extracted thus resolving all clauses");
        if (!generate_resolvents(solver, lit, antecedents0, antecedents1, &resolved, limit))
            failed = true;
    }
