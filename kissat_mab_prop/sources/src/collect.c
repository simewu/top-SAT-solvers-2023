...
static void flush_watched_clauses_by_literal(kissat *solver, litpairs *hyper, unsigned lit, bool compact,
    reference start)
{
    ...
    while (p != end_of_watches) {
        ...
        if (head.type.binary) {
            ...
            const value other_fixed = (other_value && !all_assigned[other_idx].level) ? other_value : 0;
            const unsigned mother = kissat_map_literal(solver, other, compact);
            if (lit_fixed > 0 || other_fixed > 0 || mother == INVALID_LIT) {
                if (lit < other)
                    kissat_delete_binary(solver, head.binary.redundant, head.binary.hyper, lit, other);
            } else {
                assert(!lit_fixed);
                assert(!other_fixed);
    ...
    if (lit_fixed)
        ...
        *mlit_watches = *lit_watches;
        LOG("copied watches[%u] = watches[%u] "
            "(size %" SECTOR_FORMAT ")",
            mlit, lit, mlit_watches->size);
        memset(lit_watches, 0, sizeof *lit_watches);
    } else
        assert(mlit == lit);
}

static void flush_hyper_binary_watches(kissat *solver, litpairs *hyper, bool compact)
{
    ...
    for (const litpair *p = BEGIN_STACK(*hyper); p != end; p++) {
        ...
        if (((lit_value < 0 && other_value > 0) || ((lit_value > 0 && other_value < 0)))) {
    ...
    if (flushed)
        kissat_phase(solver, "collect", GET(garbage_collections), "flushed %zu unused hyper binary clauses %.0f%%",
            flushed, kissat_percent(flushed, SIZE_STACK(*hyper)));
    (void)flushed;
}

static void flush_all_watched_clauses(kissat *solver, bool compact, reference start)
{
    assert(solver->watching);
    ...
    for (all_variables(idx)) {
        ...
        flush_watched_clauses_by_literal(solver, &hyper, not_lit, compact, start);
    }
...
static void update_large_reason(kissat *solver, assigned *assigned, unsigned forced, clause *dst)
{
    assert(dst->reason);
    ...
    assert(!a->binary);
    if (a->reason != dst_ref) {
        LOG("reason reference %u of %s updated to %u", a->reason, LOGLIT(forced), dst_ref);
        a->reason = dst_ref;
    }
    dst->reason = false;
}
...
static void get_forced_and_update_large_reason(kissat *solver, assigned *assigned, const value *values, clause *dst)
{
    const unsigned forced = get_forced(values, dst);
    update_large_reason(solver, assigned, forced, dst);
}

static void update_first_reducible(kissat *solver, const clause *end, clause *first_reducible)
{
    if (first_reducible >= end) {
        ...
        solver->first_reducible = kissat_reference_clause(solver, first_reducible);
    } else {
...
static void update_last_irredundant(kissat *solver, const clause *end, clause *last_irredundant)
{
    if (!last_irredundant) {
...
static void move_redundant_clauses_to_the_end(kissat *solver, reference ref)
{
    ...
    kissat_phase(solver, "move", GET(moved), "moving redundant clauses of %s to the end",
...
static reference sparse_sweep_garbage_clauses(kissat *solver, bool compact, reference start)
{
    assert(solver->watching);
        ...
        *(unsigned *)dst = *(unsigned *)src;
        ...
        if (new_size == 2) {
            ...
            if (dst->reason) {
                ...
                LOGBINARY(mfirst, msecond, "reason clause[%u] of %s updated to binary reason", a->reason,
                    LOGLIT(forced));
...
#ifndef QUIET
    if (flushed_literals)
        kissat_phase(solver, "collect", GET(garbage_collections), "flushed %zu falsified literals in large clauses",
            flushed_literals);
    size_t flushed_clauses = flushed_satisfied_clauses + flushed_garbage_clauses;
    if (flushed_satisfied_clauses)
        kissat_phase(solver, "collect", GET(garbage_collections), "flushed %zu satisfied large clauses %.0f%%",
            flushed_satisfied_clauses, kissat_percent(flushed_satisfied_clauses, flushed_clauses));
    if (flushed_garbage_clauses)
        kissat_phase(solver, "collect", GET(garbage_collections), "flushed %zu large garbage clauses %.0f%%",
            flushed_garbage_clauses, kissat_percent(flushed_garbage_clauses, flushed_clauses));
    kissat_phase(solver, "collect", GET(garbage_collections), "collected %s in total", FORMAT_BYTES(bytes));
#endif
    ...
    if (first_redundant && last_irredundant && first_redundant < last_irredundant) {
#ifdef LOGGING
        size_t move_bytes = (char *)dst - (char *)first_redundant;
        LOG("redundant bytes %s (%.0f%) out of %s moving bytes", FORMAT_BYTES(redundant_bytes),
            kissat_percent(redundant_bytes, move_bytes), FORMAT_BYTES(move_bytes));
#endif
...
static void dense_sweep_garbage_clauses(kissat *solver)
{
    ...
    size_t bytes = (char *)END_STACK(solver->arena) - (char *)dst;
#endif
    kissat_phase(solver, "collect", GET(garbage_collections), "flushed %zu large garbage clauses",
        flushed_garbage_clauses);
    kissat_phase(solver, "collect", GET(garbage_collections), "collected %s in total", FORMAT_BYTES(bytes));
#ifndef NMETRICS
    ADD(allocated_collected, bytes);
