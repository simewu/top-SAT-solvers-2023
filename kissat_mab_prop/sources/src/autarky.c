...
static inline void autarky_unassign(kissat *solver, value *autarky, unsigneds *work, unsigned lit)
{
...
static inline unsigned propagate_clause(kissat *solver, const value *values, value *autarky, unsigneds *work, clause *c)
{
    assert(!c->garbage);
    assert(!c->redundant);
...
static inline unsigned propagate_unassigned(kissat *solver, word *arena, const value *values, value *autarky,
...
static inline unsigned propagate_autarky(kissat *solver, const value *values, value *autarky, unsigneds *work)
{
    unsigned unassigned = 0;
    word *arena = BEGIN_STACK(solver->arena);
    while (!EMPTY_STACK(*work)) {
        unsigned lit = POP_STACK(*work);
        unassigned += propagate_unassigned(solver, arena, values, autarky, work, lit);
    }
    return unassigned;
}

static unsigned determine_autarky(kissat *solver, value *autarky, unsigneds *work)
{
    assert(!solver->level);
    ...
    if (assigned)
        kissat_phase(solver, "autarky", GET(autarky_determined), "preliminary large clause autarky of size %u",
            assigned);
    else {
        kissat_phase(solver, "autarky", GET(autarky_determined), "empty preliminary large clause autarky");
        return UINT_MAX;
    }
    ...
    for (all_literals(lit)) {
        ...
        if (EMPTY_STACK(*work))
            continue;
        const unsigned unassigned = propagate_autarky(solver, values, autarky, work);
        assert(unassigned <= assigned);
        assigned -= unassigned;
    }

    if (assigned)
        kissat_phase(solver, "autarky", GET(autarky_determined), "preliminary binary clause autarky of size %u",
            assigned);
    else {
        kissat_phase(solver, "autarky", GET(autarky_determined), "empty preliminary binary clause autarky");
        return UINT_MAX;
    }
    ...
    for (all_clauses(c)) {
        ...
        if (TERMINATED(2))
            return UINT_MAX;
        unsigned unassigned = propagate_clause(solver, values, autarky, work, c);
        if (unassigned) {
    ...
    if (assigned)
        kissat_phase(solver, "autarky", GET(autarky_determined), "final autarky of size %u", assigned);
    else
        kissat_phase(solver, "autarky", GET(autarky_determined), "empty final autarky");
...
#ifndef QUIET
    if (flushed_large)
        kissat_very_verbose(solver, "flushed %zu large clause references", flushed_large);
    if (flushed_binaries)
        kissat_very_verbose(solver, "flushed %zu autarky binary clauses", flushed_binaries);
#else
...
static void autarky_literal(kissat *solver, unsigned lit)
{
...
void kissat_autarky(kissat *solver)
{
