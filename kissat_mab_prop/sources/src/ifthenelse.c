...
static bool get_ternary_clause(kissat *solver, reference ref, unsigned *p, unsigned *q, unsigned *r)
{
...
static bool match_ternary_ref(kissat *solver, reference ref, unsigned a, unsigned b, unsigned c)
{
    clause *clause = kissat_dereference_clause(solver, ref);
    const value *values = solver->values;
    unsigned found = 0;
...
static bool match_ternary_watch(kissat *solver, watch watch, unsigned a, unsigned b, unsigned c)
{
...
static const watch *find_ternary_clause(kissat *solver, uint64_t *steps, unsigned a, unsigned b, unsigned c)
{
...
bool kissat_find_if_then_else_gate(kissat *solver, unsigned lit, unsigned negative)
{
    ...
    for (const watch *p1 = begin; steps < limit && p1 != last; p1++) {
        ...
        for (const watch *p2 = p1 + 1; steps < limit && p2 != end; p2++) {
            ...
            const watch *p3 = find_ternary_clause(solver, &steps, not_lit, b1, not_c1);
            if (!p3)
                continue;
            const unsigned not_c2 = NOT(c2);
            const watch *p4 = find_ternary_clause(solver, &steps, not_lit, b2, not_c2);
            if (!p4)
                continue;
            ...
            LOG("found if-then-else gate %s = (%s ? %s : %s)", LOGLIT(lit), LOGLIT(NOT(b1)), LOGLIT(not_c1),
                LOGLIT(not_c2));
