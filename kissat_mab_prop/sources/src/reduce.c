...
static bool collect_reducibles(kissat *solver, reducibles *reds, reference start_ref)
{
    ...
    if (redundant < solver->first_reducible)
        LOG("updating redundant clauses start from %zu to %zu", (size_t)solver->first_reducible, (size_t)redundant);
    else
        LOG("no update to redundant clauses start %zu", (size_t)solver->first_reducible);
#endif
...
#ifndef QUIET
    // *INDENT-OFF*
    size_t total_hyper_ternary_clauses = flushed_hyper_ternary_clauses + used_hyper_ternary_clauses;
    if (flushed_hyper_ternary_clauses)
        kissat_phase(solver, "reduced", GET(reductions), "reduced %zu unused hyper ternary clauses %.0f%% out of %zu",
            flushed_hyper_ternary_clauses, kissat_percent(flushed_hyper_ternary_clauses, total_hyper_ternary_clauses),
...
{
    RADIX_STACK(RADIX_SORT_REDUCE_LENGTH, reducible, uint64_t, *reds, USEFULNESS);
}

static void mark_less_useful_clauses_as_garbage(kissat *solver, reducibles *reds)
{
    const size_t size = SIZE_STACK(*reds);
    size_t target = size * (GET_OPTION(reducefraction) / 100.0);
#ifndef QUIET
    statistics *statistics = &solver->statistics;
    const size_t clauses = statistics->clauses_irredundant + statistics->clauses_redundant;
    kissat_phase(solver, "reduce", GET(reductions),
        "reducing %zu (%.0f%%) out of %zu (%.0f%%) "
        "reducible clauses",
        target, kissat_percent(target, size), size, kissat_percent(size, clauses));
#endif
    unsigned reduced = 0;
...
static bool compacting(kissat *solver)
{
    ...
    LOG("%u inactive variables %.0f%% <= limit %u %.0f%%", inactive, kissat_percent(inactive, solver->vars), limit,
...
int kissat_reduce(kissat *solver)
{
    ...
    INC(reductions);
    LOG("beginreduce");
    kissat_phase(solver, "reduce", GET(reductions), "reduce limit %" PRIu64 " hit after %" PRIu64 " conflicts",
    ...
    if (start != INVALID_REF) {
        ...
        kissat_phase(solver, "reduce", GET(reductions), "reducing clauses after offset %zu in arena", start);
        kissat_phase(solver, "reduce", GET(reductions), "sweeping %zu words %s %.0f%%", words_to_sweep,
            FORMAT_BYTES(bytes_to_sweep), kissat_percent(words_to_sweep, arena_size));
#endif
