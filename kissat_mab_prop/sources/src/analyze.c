...
static bool one_literal_on_conflict_level(kissat *solver, clause *conflict, unsigned *conflict_level_ptr)
{
    assert(conflict);
    ...
    LOG("found %u literals on conflict level %u", literals_on_conflict_level, conflict_level);
    *conflict_level_ptr = conflict_level;
    ...
    if (conflict_level < solver->level) {
        LOG("forced backtracking due to conflict level %u < level %u", conflict_level, solver->level);
...
static void mark_literal_as_analyzed(kissat *solver, assigned *all_assigned, unsigned lit, const char *type)
{
    const unsigned idx = IDX(lit);
...
static inline void analyze_reason_side_literals(kissat *solver)
{
    ...
    for (all_stack(unsigned, lit, solver->clause.lits)) {
        ...
        if (a->reason == DECISION)
            continue;
        if (a->binary)
            mark_literal_as_analyzed(solver, all_assigned, a->reason, "reason side");
        else {
            assert(a->reason < SIZE_STACK(solver->arena));
            clause *c = (clause *)(arena + a->reason);
            for (all_literals_in_clause(lit, c))
                if (IDX(lit) != idx)
                    mark_literal_as_analyzed(solver, all_assigned, lit, "reason side");
        }
    }
}
...
static void reset_markings(kissat *solver)
{
...
static void reset_analyze(kissat *solver)
{
...
static void update_trail_average(kissat *solver)
{
    ...
    LOG("trail filled %.0f%% (size %u, unflushed %u, active %u)", filled, size, solver->unflushed, active);
    UPDATE(trail, filled);
}
